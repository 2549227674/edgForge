...
> 历史继承层（只读归档）：[`docs/research/`](research/README.md) 中的 04 事实台账、R1–R4 调研报告与面经库。它们提供来源和当时判断，不再滚动修订；与实测或 `docs/facts.md` 冲突时以后两者为准。13 号发布卫生原则仍按本蓝图相关条款继承。
...
| 校准 | RTN → GPTQ → AWQ → SmoothQuant → QAT | GGUF 自带 imatrix 校准；M0 W8A8 实测使用 RKLLM 官方 19 条，M2 若启动须独立定义 agent 域正式校准集；QAT 见决策 3 |
