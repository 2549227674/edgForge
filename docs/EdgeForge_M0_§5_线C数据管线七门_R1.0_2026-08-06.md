# EdgeForge M0 §5：线 C · 数据管线七门（R1.1 · 事实版）
...
所有来源均从 Hugging Face `refs/convert/parquet` 导出并冻结到 [manifest](../manifests/data_archive_sha256.json)。该 manifest 保存每个文件的 SHA-256、来源 revision、上游/本地行数、许可和文件角色；默认 Dataset Viewer revision 的观察值另存于 `data/pipeline/gate1_upstream_metadata.json`，不与冻结 export revision 混用。
...
| `data/pipeline/gate3_scan_payloads/`、`gate7_render_samples/` | 安全投影及逐 token 人工审核材料 | 本地忽略；前者可能含敏感载荷，后者体积大且包含训练文本片段。 |
